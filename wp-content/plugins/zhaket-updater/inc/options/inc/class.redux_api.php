    ...
    if ( ! class_exists( 'Zhaket\Redux' ) ) {
        ...
        class Redux {
            ...
            public static function load() {
                add_action( 'after_setup_theme', array( 'Zhaket\Redux', 'createRedux' ),10);
                add_action( 'init', array( 'Zhaket\Redux', 'createRedux' ));
                add_action( 'switch_theme', array( 'Zhaket\Redux', 'createRedux' ));
            }

            public static function init( $opt_name = "" ) {
                if ( ! empty( $opt_name ) ) {
                    self::loadRedux( $opt_name );
                    remove_action( 'setup_theme', array( 'Zhaket\Redux', 'createRedux' ) );
                }
            }
            ...
            public static function loadExtensions( $ReduxFramework ) {
                if ( $instanceExtensions = self::getExtensions( $ReduxFramework->args['opt_name'], "" ) ) {
                    foreach ( $instanceExtensions as $name => $extension ) {
	                    $extension['class']='Zhaket\\'.$extension['class'];
            ...
            public static function loadRedux( $opt_name = "" ) {
                ...
                if ( ! class_exists( 'Zhaket\ReduxFramework' ) ) {
                ...
                if ( isset( self::$uses_extensions[ $opt_name ] ) && ! empty( self::$uses_extensions[ $opt_name ] ) ) {
                    add_action( "redux/extensions/{$opt_name}/before", array( 'Zhaket\Redux', 'loadExtensions' ), 0 );
                }
            ...
            public static function checkExtensionClassFile( $opt_name, $name = "", $class_file = "", $instance = "" ) {
                if ( file_exists( $class_file ) ) {
                    ...
                    if ( file_exists( $api_check ) && ! class_exists( 'Zhaket\Redux_' . ucfirst( $name ) ) ) {
