jQuery(function () {
	...
	if(dpc.length){/**/
		...
		jQuery.ajax({
			...
			data: {
				action: 'digits_check_purchasecode',
				code: code,
			...
			success: function (data) {
                ...
                if (data == 0) {
                    ...
                    if (sgs.attr("ajxsu")) {
						...
						if(jQuery('#dig_activated').val() != 1){
							alert('افزونه فعال شد. صفحه رفرش می شود.');
							jQuery('.dig_ecr_desc').html('**افزونه فعال شد. صفحه رفرش می شود.');
              digits_redirect('-1');
              refreshCode = 1;
							return true;
						}
                    ...
                    } else showDigErrorMessage(digsetobj.Error);
    ...
    jQuery("#digits_setting_update button[type='submit']").on('click', function () {
        ...
        jQuery("." + te + "cred").find("input").each(function (e) {
    ...
    function show_field_options(type, options, values) {
        ...
        if (values != null) {
            ...
            required_field_box.find('select').val(values['required']);
    ...
    /************Addons*************/
	jQuery(".addontabview").each(function(){
		var c = jQuery(this).attr('tab');
		jQuery(this).attr('id',c);
	})
    jQuery(".addontabview").click(function(){
        var c = jQuery(this).attr('tab');
        jQuery(".digaddonscurrentactive").removeClass("digaddonscurrentactive").hide();
        var tab = jQuery("."+c);
        tab.fadeIn(150).addClass("digaddonscurrentactive");
        jQuery(".dig-addon-tab-active").removeClass("dig-addon-tab-active");
        jQuery(this).addClass("dig-addon-tab-active");
        updateURL("subtab",c.slice(0,-3));
        return false;
    });
    jQuery(".addoninstalleditem").click(function(){
        var c = jQuery(this).attr('tab');
        jQuery(".digaddonscurrentactive").removeClass("digaddonscurrentactive").hide();
        var tab = jQuery("."+c);
        tab.fadeIn(150).addClass("digaddonscurrentactive");
        jQuery(".dig-addon-tab-active").removeClass("dig-addon-tab-active");
        jQuery("#"+c).addClass("dig-addon-tab-active");
        updateURL("subtab",c.slice(0,-3));
        return false;
    });
