 ...
 * Author: Digits فارسی (پرشین اسکریپت) - پیام یزدانیان
 * Text Domain: digits
 * Requires PHP: 5.5
 * Domain Path: /languages
 */

//add_action('wp_head',printDigitOptions);
function printDigitOptions(){
    $gatewayToUse = get_option('digit_tapp', 1);
    $gatewayToUse = apply_filters('digit_gateway_to_use', $gatewayToUse, "+90");
    echo "printDigitOptions(digit.php:16) : '".$gatewayToUse."'";
        ...
        if ($required == 1 && empty($e_value)) {
            ...
            if ($type == "captcha") {
                ...
                if ($type == "dropdown" || $type == "radio") {
                    ...
                    if ($required == 0 && (empty($e_value) || ($type=='dropdown' && $e_value==-1))) {
                        continue;
